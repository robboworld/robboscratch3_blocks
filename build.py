...
if sys.version_info[0] < 3:
  raise Exception("Blockly build requires Python 3.x.\n"
...
VENDORED_CLOSURE_LIBRARY = "closure-library"

def resolve_closure_library_search_dir(closure_root, closure_library):
  """Return Closure Library root for calcdeps.

  The npm package google-closure-library is incomplete (no goog.useragent, etc.).
  Prefer the full vendored tree in ./closure-library when present.
  """
  vendored_useragent = os.path.join(
      VENDORED_CLOSURE_LIBRARY, "closure", "goog", "useragent")
  if os.path.isdir(vendored_useragent):
    return VENDORED_CLOSURE_LIBRARY
  npm_path = os.path.join(closure_root, closure_library)
  npm_useragent = os.path.join(npm_path, "closure", "goog", "useragent")
  if os.path.isdir(npm_useragent):
    return npm_path
  return npm_path

def build_force_enabled():
  return os.environ.get("BLOCKS_BUILD_FORCE") == "1" or "--force" in sys.argv

def collect_js_files(paths):
  """Collect .js files from search path entries (files or directories)."""
  collected = []
  for entry in paths:
...
def needs_rebuild(dest, srcs):
  """True when dest is missing or any source is newer than dest."""
  if build_force_enabled():
    return True
  ...
  try:
    ...
    return True
  if os.path.getmtime(__file__) > dest_mtime:
    return True
  for src in srcs:
    try:
      if os.path.getmtime(src) > dest_mtime:
        return True
    except OSError:
      return True
  return False
...
class BuildThread(threading.Thread):
  """Thread base that records build failures for the main process."""
...
class Gen_uncompressed(BuildThread):
  ...
  def run_build(self):
    ...
    srcs = collect_js_files(self.search_paths)
    if not needs_rebuild(target_filename, srcs):
      print("SKIP (up to date): " + target_filename)
      return
...
class Gen_compressed(BuildThread):
  ...
  def gen_core(self, vertical):
    ...
    srcs = collect_js_files(search_paths)
    if not needs_rebuild(target_filename, srcs):
      print("SKIP (up to date): " + target_filename)
      return
  ...
  def gen_blocks(self, block_type):
    ...
    srcs = list(filenames)
    srcs.append(os.path.join("build", "gen_blocks.js"))
    srcs.append(os.path.join("core", "colours.js"))
    srcs.append(os.path.join("core", "constants.js"))
    if not needs_rebuild(target_filename, srcs):
      print("SKIP (up to date): " + target_filename)
      return
    # Define the parameters for the POST request.
...
if __name__ == "__main__":
  ...
  # ExpandDirectories returns a map() iterator in Python 3; materialize so two
  # filter() passes do not share one exhausted iterator.
  closure_library_dir = resolve_closure_library_search_dir(
      closure_root, closure_library)
  search_paths = list(calcdeps.ExpandDirectories(
      ["core", closure_library_dir]))
  ...
  # Run all tasks in parallel threads, then wait — otherwise blockly_compressed_*
  # may not be written before npm/webpack continues (or failures stay silent).
  build_threads = []
  build_threads.append(Gen_uncompressed(search_paths_vertical, True, closure_env))
  build_threads.append(Gen_uncompressed(search_paths_horizontal, False, closure_env))
  build_threads.append(Gen_compressed(
      search_paths_vertical, search_paths_horizontal, closure_env))
  ...
  failed = [thread for thread in build_threads if thread.build_error]
  if failed:
    for thread in failed:
      print("BUILD FAILED:", thread.build_error)
    sys.exit(1)
